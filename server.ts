...
interface ScanPayload {
  ...
  gyro_aligned?: boolean;
  buru_card_detected?: boolean;
 ...
 */
function generateBuruId(length: number, width: number, instep: number): string {
 ...
 */
function validateMeasurements(length: number, width: number, instep: number): { valid: boolean; error?: string } {
  if (length < 180 || length > 350) return { valid: false, error: 'Length out of range (180-350mm)' };
  if (width < 60 || width > 150) return { valid: false, error: 'Width out of range (60-150mm)' };
  if (instep < 40 || instep > 130) return { valid: false, error: 'Instep out of range (40-130mm)' };
  return { valid: true };
}
 ...
 * }
 */
app.post('/api/scan', scanLimiter, async (req: express.Request, res: express.Response) => {
  try {
    const payload: ScanPayload = req.body;
 ...
 * }
 */
app.get('/api/profile/:buru_id', async (req: express.Request, res: express.Response) => {
  try {
 ...
 * }
 */
app.post('/api/match', matchLimiter, async (req: express.Request, res: express.Response) => {
  try {
 ...
 * Quick health check
 */
app.get('/api/healthcheck', (req: express.Request, res: express.Response) => {
